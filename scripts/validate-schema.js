...
async function validate() {
  ...
  try {
    const genericSchemaString = generateSchema(null, suburbs);
  ...
  for (const suburb of suburbs) {
    try {
      const suburbSchemaString = generateSchema(suburb, suburbs);
      JSON.parse(suburbSchemaString); // The core validation check.
    } catch (e) {
      console.error(`❌ FATAL: Schema for "${suburb.name}" failed validation!`, e);
      invalidCount++;
    }
  }
